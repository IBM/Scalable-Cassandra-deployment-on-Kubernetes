...
利用 Kubernetes 概念，比如 [PersistentVolume](https://kubernetes.io/docs/concepts/storage/persistent-volumes/) 和 [StatefulSet](https://kubernetes.io/docs/concepts/workloads/controllers/statefulset/)，我们可以提供一种具有灾备能力的 Cassandra 安装，并确信它的数据（状态）是安全的。

我们还对 Cassandra 使用了一个[“无头”服务](https://kubernetes.io/docs/concepts/services-networking/service/#headless-services)。这样，我们就可以为应用程序提供一种通过 [KubeDNS](https://kubernetes.io/docs/concepts/services-networking/dns-pod-service/) 访问它的方式，而且不会将它暴露给外部世界。要从您的开发人员工作站访问它，您可以对任何 Cassandra Pod 使用 `kubectl exec` 命令。如果您确实希望将应用程序连接到它，那么您可以在配置您的应用程序时使用 KubeDNS 值 `cassandra.default.svc.cluster.local`。
...
* [Bluemix Container 服务](https://console.ng.bluemix.net/catalog/?taxonomyNavigation=apps&category=containers)
...
* [IBM Bluemix Container 服务](https://github.com/IBM/container-journey-template#container-journey-template---creating-a-kubernetes-cluster)，部署在一个 IBM 管理的集群中（免费的小集群）
...
_这里的代码会定期使用 Travis 针对[来自 Bluemix Container 服务的 Kubernetes 集群](https://console.ng.bluemix.net/docs/containers/cs_ov.html#cs_ov) 进行测试。_
...
## 创建一个 Cassandra 服务来实现 Cassandra 集群编队和发现

### 1.创建一个 Cassandra 无头服务

为了使我们能对 Cassandra 种子节点（我们很快将部署它）执行简单的发现操作，我们可以创建一个“无头”服务。  为此，我们为 [cassandra-service.yaml](cassandra-service.yaml) 中的 **clusterIP** 指定 **none**。这个无头服务允许我们对 Pod 使用 KubeDNS，以发现 Cassandra 种子的 IP 地址。

可以使用 [cassandra-service.yaml](cassandra-service.yaml) 文件创建这个无头服务：
...
部署到 Kubernetes 的大部分应用程序都应该是云原生的，而且依靠外部资源来存储其数据（或状态）。但是，因为 Cassandra 是一个数据库，所以我们可以使用有状态集和持久卷来确保数据库中的灾备能力。

### 2.创建本地卷

要创建持久 Cassandra 节点，需要配备持久卷。可通过两种方式配备持久卷：**动态和静态**。

出于简单性和兼容性，我们将使用**静态**配备功能，使用所提供的 yaml 文件手动创建卷。

_备注：您将需要与 Cassandra 节点数量相同的持久卷。如果您希望有 3 个 Cassandra 节点，则需要创建 3 个持久卷。_
...
要检查 Cassandra 节点是否在正常运行，可以执行 **nodetool status：**
